...
From inside an allocated VM you can SSH back to the gateway to get another VM. Each VM has a client key generated at boot and an injected SSH config pointing back to the gateway, so recursive allocation works out of the box.
