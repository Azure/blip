...
# installs KubeVirt when needed, builds the blip image, loads it into kind,
...
# ---------------------------------------------------------------------------
# 3. Build the blip container image
# ---------------------------------------------------------------------------
...
if [[ -n "$POOL_REPLICAS" ]]; then
    ...
    envsubst '${REGISTRY} ${BLIP_TAG}' < "$REPO_ROOT/manifests/pool.yaml" \
        | sed "s/replicas: [0-9]*/replicas: ${POOL_REPLICAS}/" \
        | kubectl apply -f -
else
    envsubst '${REGISTRY} ${BLIP_TAG}' < "$REPO_ROOT/manifests/pool.yaml" | kubectl apply -f -
fi

# ---------------------------------------------------------------------------
# 6. Restart deployments to pick up the new image
# ---------------------------------------------------------------------------
